...
function Farm() {
  ...
  const handleInputChange = (e) => {
    ...
    } else if (name in farmingCostForm) {
      setFarmingCostForm((prev) => ({
        ...prev,
        [name]: value,
      }));
    } else if (name in farmReport) {
      setFarmReport((prev) => ({
        ...prev,
        [name]: value,
      }));
  ...
  const handleSubmit = (e) => {
    ...
    const fullReport = {
      ...
      inspection,
      farmReport,
      id: Date.now(), // unique ID for later retrieval
    };

    const existingReports = JSON.parse(localStorage.getItem("farmReports")) || [];
    const updatedReports = [...existingReports, fullReport];
    localStorage.setItem("farmReports", JSON.stringify(updatedReports));

    setShowModal(true); // or redirect to reports
  };
  ...
  return (
    <>
      <div className="flex min-h-screen bg-green-50  h-screen overflow-hidden">
        ...
        {/* Main Content */}
        <main className="flex-1 md:p-4 bg-green-700 overflow-y-auto border-green-700">
          <div className="bg-green-50 rounded-2xl border-2 border-white mx-auto px-2 sm:px-4 py-4 sm:py-8 flex flex-col items-center justify-center gap-8 min-h-screen w-full">
            ...
            <div className="w-full max-w-4xl bg-white shadow-lg rounded-xl p-2 sm:p-6 mt-13 sm:mt-2">
              ...
              >
                ...
                <span className="font-semibold">Report Tittle</span>
                <input type="text"
                   name="reportTitle"
                   value={farmReport.reportTitle}
                   onChange={handleInputChange}
                  className="flex mt-2 w-[300px] p-3 border border-green-200 rounded-lg bg-gray-100 mb-5 sm:mb-2"
                />
                <span className="font-semibold">Report Category</span>
                <input type="text" 
                  name="reportCategory"
                  value={farmReport.reportCategory}
                   onChange={handleInputChange}
                  className="flex mt-2 w-[300px] p-3 border border-green-200 rounded-lg bg-gray-100 mb-5 sm:mb-2"
                />
                <span className="font-semibold">Report Zone</span>
                <input type="text" 
                  name="reportZone"
                  value={farmReport.reportZone}
                   onChange={handleInputChange}
                  className="flex mt-2 w-[300px] p-3 border border-green-200 rounded-lg bg-gray-100 mb-5 sm:mb-2"
                />
                <span className="font-semibold">Report Discription</span>
                <input type="text" 
                  name="reportDescription"
                  value={farmReport.reportDescription}
                  onChange={handleInputChange}
                  className="flex mt-2 w-[300px] h-[100px] p-3 border border-green-200 rounded-lg bg-gray-100 mb-5 sm:mb-2"
                />
                <span className="font-semibold">Report Date</span>
                <input type="date" 
                  name="reportDate"
                  value={farmReport.reportDate}
                   onChange={handleInputChange}
                  className="flex mt-2 w-[190px] p-3 border border-green-200 rounded-lg bg-gray-100 mb-5 sm:mb-2"
                />
                <span className="font-semibold">Crop Type</span>
                <input type="text" 
                  name="cropType"
                  value={farmReport.cropType}
                  onChange={handleInputChange}
                  className="flex mt-2 w-[300px] p-3 border border-green-200 rounded-lg bg-gray-100 mb-5 sm:mb-2"
                />
                ...
                <input type="text" 
                  name="reportLocation"
                  value={farmReport.reportLocation}
                  onChange={handleInputChange}
                  className="flex mt-2 w-[300px] p-3 border border-green-200 rounded-lg bg-gray-100 mb-5 sm:mb-2"
                />
                  </div>
              ...
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
                ...
                <div className="relative bg-white/90 backdrop-blur-md rounded-xl shadow-lg p-8 w-full max-w-sm text-center">
                  ...
                  <p className="text-green-800 mb-4">
                    Farming cost and Report has been successfully updated!!!
                  </p>
                  <button
