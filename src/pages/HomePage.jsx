...
import Farmtruck from "../assets/farm-truck.png";
...
function HomePage() {
  ...
  const impactImages = [Agric1, Agric2, Agric3, Agric4, Agric5];
  ...
  return (
    <>
      ...
      <section>
        <div
          className="relative w-full min-h-[90vh] bg-no-repeat bg-cover bg-center flex items-center justify-center overflow-x-hidden animate-fade-in-down"
          style={{ backgroundImage: `url(${Bgimg})` }}
        >
          <div className="absolute inset-0 bg-black/50 transition-opacity duration-700" />
          <div className="relative z-10 text-center px-4">
            <p className="text-2xl italic sm:text-3xl md:text-4xl lg:text-4xl text-green-200 mb-5 font-semibold leading-tight animate-fade-in">
              Empowering Farmers. Enabling Agents. Transforming
            </p>
            <h1 className="text-5xl sm:text-7xl md:text-8xl lg:text-9xl xl:text-[8rem] font-extrabold text-white drop-shadow-lg mb-6 animate-fade-in-up">
              AGRICULTURE
            </h1>
            <div className="flex flex-col sm:flex-row gap-4 justify-center animate-fade-in-up">
              <Link to="/sign_up">
                <button className="bg-green-700 text-white px-6 py-3 rounded-lg cursor-pointer hover:bg-green-600 transition text-lg font-semibold shadow-lg hover:scale-105 duration-300">
                  Get Started
                </button>
              </Link>
              <Link to="/login">
                <button className="bg-white text-green-700 border border-green-600 px-6 py-3 cursor-pointer rounded-lg hover:bg-green-100 transition text-lg font-semibold shadow-lg hover:scale-105 duration-300">
                  Sign In
                </button>
              </Link>
            </div>
      ...
      <section className="animate-fade-in-up">
        <div className="min-h-[70vh] flex flex-col items-center justify-center shadow bg-green-50 px-4">
          <div className="max-w-2xl w-full">
            <h2 className="text-3xl font-bold text-center text-green-700 mb-10 animate-fade-in-down">
              Welcome to The Extension Manager
            </h2>
            <p className="text-black text-lg text-center sm:text-xl animate-fade-in">
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="flex flex-wrap justify-center gap-8">
            {[
              {
                icon: "📲",
                title: "A Mobile-First Platform",
                desc: "For real-time advisory, recordkeeping and farm monitoring - even offline.",
              },
              {
                icon: "🧑‍🌾",
                title: "Trained Youth Agents",
                desc: "Embedded in communities, delivering services in local languages.",
              },
              {
                icon: "📈",
                title: "Smart Farm Insights",
                desc: "Powered by AI, satellite imagenary and on-ground data collection.",
              },
              {
                icon: "💼",
                title: "Access to Inputs, Credits, Insurance, and Markets",
                desc: "Fully integrated into the extension workflow.",
              },
            ].map((item, idx) => (
              <div
                key={item.title}
                className={`flex-1 min-w-[250px] max-w-xs bg-green-50 p-8 rounded-xl shadow hover:shadow-2xl transition-all duration-500 hover:scale-105 group ${
                  idx % 2 === 0
              ...
              >
                ...
                <h3 className="text-xl font-semibold mb-2 group-hover:text-green-800 transition-colors duration-300">
                  {item.title}
                </h3>
      ...
      <section className="animate-fade-in-up">
        <div className="min-h-[90vh] flex flex-col-reverse lg:flex-row items-center justify-center bg-white px-4 py-12 mb-10 gap-8">
          {/* Image Section */}
          <div className="w-full lg:w-1/2 flex justify-center mb-8 lg:mb-0 animate-slide-in-left">
            <img
              src={Farmtruck}
              alt="Farm Truck"
              className="w-full max-w-xs sm:max-w-sm md:max-w-md rounded-xl shadow-lg object-cover hover:scale-105 transition-transform duration-500"
            />
          </div>
          {/* Text Section */}
          <div className="w-full lg:w-1/2 max-w-2xl text-center lg:text-left mx-5 animate-slide-in-right">
            ...
            <p className="text-base sm:text-lg text-gray-700 mb-2">
              <b>Deliver Services</b> - From planting advice to financial access,
            ...
            <h1 className="text-xl sm:text-xl font-bold text-gray-700 my-10 animate-fade-in">
      ...
      <section className="animate-fade-in-up">
        ...
        >
          ...
          <div className="relative z-10 w-full max-w-4xl mx-auto flex flex-col items-center">
            <p className="text-white text-2xl sm:text-3xl md:text-4xl font-normal italic mb-8 text-center drop-shadow-lg animate-fade-in-down">
              ...
              {impactImages.map((img, idx) => (
                ...
                >
                  <img
                    src={img}
                    alt=""
                    className="w-64 h-64 sm:w-40 sm:h-40 md:w-44 md:h-44 object-cover rounded-xl shadow-lg bg-white group-hover:scale-110 transition-transform duration-500"
                  />
                </div>
              ))}
            </div>
      ...
      <section className="min-h-screen bg-green-50 py-16 px-4 flex flex-col items-center text-center animate-fade-in-up">
        <h1 className="text-4xl font-bold text-green-700 mb-4 animate-fade-in-down">
          🎯Impact Goals
        </h1>
        <p className="text-lg text-gray-700 mb-10 max-w-2xl mx-auto animate-fade-in">
        ...
        <div className="flex flex-row flex-wrap justify-center gap-8 w-full max-w-5xl">
          {goals.map((goal, idx) => (
            <div
              key={idx}
              className={`bg-white rounded-xl shadow-lg p-8 flex flex-col items-center hover:shadow-2xl transition min-w-[220px] max-w-xs flex-1 text-center animate-fade-in-up hover:scale-105 duration-300`}
      ...
      <section className="animate-fade-in-up">
        <div
          ...
          style={{ backgroundImage: `url(${Ourimptbg})` }}
        >
          <p className="text-white sm:text-3xl md:text-4xl font-normal italic mb-8 text-center drop-shadow-lg animate-fade-in-down">
          ...
          <img
            src={Susagric}
            alt=""
            className="rounded-lg w-[90vw] max-w-4xl h-auto animate-fade-in-up"
          />
        </div>
      </section>
